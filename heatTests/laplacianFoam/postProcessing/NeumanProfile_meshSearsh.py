...
def processIter(rCells):
    ...
    deltaFT = np.sum(np.abs(Temp - thermo.Neuman_sol(r_grid))) / (mesh_shape[0] * mesh_shape[1] * mesh_shape[2])
    deltaST = np.sqrt(np.sum((Temp - thermo.Neuman_sol(r_grid)) ** 2) / (mesh_shape[0] * mesh_shape[1] * mesh_shape[2]))

    np.savetxt(f"Neuman_laplacianFoam_{rCells}.txt", np.array([rCells, deltaFT, deltaST, dt]))
...
if __name__ == "__main__":
    ...
    for rCells in rCells_:
        _, deltaFT, deltaST, dt = np.loadtxt(f"Neuman_laplacianFoam_{rCells}.txt")
        deltaFTs[rCells - rCells_[0]] = deltaFT
        deltaSTs[rCells - rCells_[0]] = deltaST
        Time[rCells - rCells_[0]] = dt

        os.remove(f"Neuman_laplacianFoam_{rCells}.txt")

    np.savetxt("Neuman_laplacianFoam.txt", np.array([rCells_, 
                                          deltaFTs, 
                                          deltaSTs,
                                          Time]).T)