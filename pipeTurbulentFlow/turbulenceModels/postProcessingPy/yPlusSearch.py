...
def processIter(i):
    ...
    setTurbModel(path, turbulenceModels[i])
    result_filename = "Data/" + turbulenceModels[i] + ".txt"
    ...
    for i in range(n):
        ...
        np.savetxt(result_filename, np.array([Re[:i+1], Lambda(p, Re)[:i+1], yPlus[0, :i+1], yPlus[1, :i+1], yPlus[2, :i+1]]).T)

    shutil.rmtree(path)

if __name__ == "__main__":
    with Pool(processes=6) as pool:  # По умолчанию использует все ядра CPU
        pool.map(processIter, range(len(turbulenceModels)))