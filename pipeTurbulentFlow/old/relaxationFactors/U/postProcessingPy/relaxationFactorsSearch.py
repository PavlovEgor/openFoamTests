...
def processIter(i, meshType=1):
    ...
    formating.setrelaxationFactors(path, relaxationFactors[i])
    ...
    for i in range(n):
        ...
        np.savetxt(result_filename, np.array([Re[:i+1], hydro.Lambda(p, Re)[:i+1], yPlus[0, :i+1], yPlus[1, :i+1], yPlus[2, :i+1], Tim[:i+1]]).T)
