...
L = (Ra * (mu ** 2) / (g * beta * dT * Pr * (rho ** 2))) ** (1/3)
U0 = lam / (Cp * rho * L)
...
print("СКОРОСТИ")
print("-" * 50)
print("Минимальное U:     {:.4f}".format(-np.min(Ux[:, Uy.shape[0]//2]) / U0))
print("Максимальное U:    {:.4f}".format(np.max(Ux[:, Uy.shape[0]//2]) / U0))
print("Минимальное U:     {:.4f}".format(-np.min(Uy[Ux.shape[0]//2, :]) / U0))
print("Максимальное U:    {:.4f}".format(np.max(Uy[Ux.shape[0]//2, :]) / U0))
print("=" * 50)
