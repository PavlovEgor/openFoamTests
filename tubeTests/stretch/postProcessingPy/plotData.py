...
for i in range(m):
    ...
    ax1.plot(R_CFD, Lambda_CFD, marker[i % 4], c=color_map(i), label=f"{i}, {round(stretch[i], 3)}")
    ax2.plot(R_CFD, yPlusAve, 'x', c=color_map(i), label="y+ " + f"{i}, {round(stretch[i], 3)}")
...
for i in range(m):
    ...
    try:
        ...
        ax1.plot(R_CFD, np.abs(Lambda_CFD - PL[:Lambda_CFD.shape[0]]), marker[i % 4], c=color_map(i), label=f"{i}, {round(stretch[i], 3)}")
        ax2.plot(R_CFD, yPlusAve, 'x', c=color_map(i), label="y+ " + f"{i}, {round(stretch[i], 3)}")
    except:
        pass
